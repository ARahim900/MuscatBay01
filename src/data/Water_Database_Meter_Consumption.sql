-- =====================================================
-- COMPLETE WATER DATABASE METER CONSUMPTION DATA
-- For Supabase Import
-- =====================================================
...
-- Zone_03_(A) L4 Apartments (sample - add all from your data)
...
SELECT 'Total meters:' as description, COUNT(*) as count FROM water_meters
UNION ALL
SELECT 'Zone_08 meters:', COUNT(*) FROM water_meters WHERE zone = 'Zone_08'
UNION ALL
SELECT 'Zone_05 meters:', COUNT(*) FROM water_meters WHERE zone = 'Zone_05'
UNION ALL
SELECT 'Zone_03_(A) meters:', COUNT(*) FROM water_meters WHERE zone = 'Zone_03_(A)'
UNION ALL
SELECT 'Zone_03_(B) meters:', COUNT(*) FROM water_meters WHERE zone = 'Zone_03_(B)';

-- Zone summary
SELECT zone, label, COUNT(*) as meter_count,
       SUM(COALESCE(jan_25,0) + COALESCE(feb_25,0) + COALESCE(mar_25,0) + 
           COALESCE(apr_25,0) + COALESCE(may_25,0) + COALESCE(jun_25,0)) as total_6month_consumption
FROM water_meters
GROUP BY zone, label
ORDER BY zone, label;